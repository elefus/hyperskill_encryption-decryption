...
interface RoboFactory {

    Robot create(String name, String description, int power);
}

class RobotFactory {

    private final Map<RobotType, RoboFactory> factories = Map.of(
            RobotType.ROBOT_CLEANER, RobotCleaner::new,
            RobotType.ROBOT_GUARDIAN, RobotGuardian::new
    );

    /** Factory method */
    public Robot getRobot(RobotType type, String name, String description, int power) {
        return factories.get(type).create(name, description, power);
    }
}
